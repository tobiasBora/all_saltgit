...
if ! id -u "$gituser" > /dev/null 2>&1 ; then
	...
	*)
		...
		[yY][eE][sS]|[yY])
		    ...
		    echo "Host ${gituser}_${serveraddr}_${port}"
		    echo "     User ${gituser}"
...
echo "If you set a password, then just use:"
echo "  $ git remote add saltserver ssh://${gituser}@${serveraddr}:${port}/${pathgitrepo}"
echo "else, if you configured a public key for the user ${gituser}, then use"
echo "the same name as the one used in your ~/.ssh/config like:"
echo "  $ git remote add saltserver ${gituser}_${serveraddr}_${port}:/${pathgitrepo}"
echo ""
